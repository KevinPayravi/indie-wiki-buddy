...
* `destination_platform`: The wiki software used by the wiki being redirected to. Currently, the only fully supported options are `mediawiki` and `dokuwiki`. `wikidot` is partially supported, depending on the search tool used by the individual wiki. If you are contributing a redirect to a wiki that is on another wiki platform, please open an issue so that support for the platform can be added.
